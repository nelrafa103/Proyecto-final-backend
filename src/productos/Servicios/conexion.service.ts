...
export class ConexionService {
   ...
   insertConexion(arg: {Nombre: string}) {
     this.ClienteRepository.query(`INSERT INTO Conexion (nombre) VALUES ('${arg.Nombre}');`)
   }
   updateConexionbyId(arg: {Id: number, Nombre: string}) {
      this.ClienteRepository.query(`UPDATE Conexion SET nombre = '${arg.Nombre}' WHERE Id_Conexion = '${arg.Id}';`)
   }
}
